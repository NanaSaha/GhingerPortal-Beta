...
# disable user
  ...
  def update_credential
     respond_to do |format|
       @user = User.find(current_user.id)
        if @user.update!(user_params)
          format.js {flash.now[:notice] = 'Credentials updated successfully. '}
          format.html { redirect_to @user, notice: 'User was successfully updated.' }
          format.json { render :show, status: :ok, location: @user }
        else
          format.js { render :user_credential}
          @user_id = current_user.id
          @user = User.find(@user_id)
          format.html { render :edit }
          format.json { render json: @user.errors, status: :unprocessable_entity }
  ...
  private
    ...
    def user_params
      params.require(:user).permit(:email, :password, :password_confirmation, :username, :person_id, :id, :username, :active_status, :del_status, :role_id )
    end
